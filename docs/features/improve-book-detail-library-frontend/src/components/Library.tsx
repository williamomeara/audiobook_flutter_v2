...
export function Library({ onBookSelect, onShowFreeBooks }: LibraryProps) {
      ...
      <div className="px-5 pt-14 pb-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-colors ${
                showFilters ? 'bg-[#f59e0b] text-black' : 'bg-[#1a2332]'
              }`}
            >
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-4">
            <button
              ...
              className={`text-sm ${activeTab === 'all' ? 'text-[#f59e0b]' : 'text-gray-400'}`}
            >
              All
            </button>
            <button
              onClick={() => setActiveTab('favorites')}
              className={`text-sm ${activeTab === 'favorites' ? 'text-[#f59e0b]' : 'text-gray-400'}`}
            >
              Favorites
            </button>
          </div>
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2 rounded-lg bg-[#1a2332] text-sm text-gray-400 hover:text-white transition-colors"
            >
              Free Books
            </button>
            <button
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#1a2332] rounded-2xl pl-12 pr-4 py-4 text-white placeholder-gray-400 outline-none"
          />
        </div>

        {/* Sort Options */}
        {showFilters && (
          <div className="mt-4 bg-[#1a2332] rounded-xl p-4">
              ...
              {['recent', 'title', 'progress'].map((option) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm capitalize ${
                    sortBy === option ? 'bg-[#f59e0b] text-black' : 'bg-[#0f1419] text-gray-400'
                  }`}
                >
                  {option}
                </button>
      ...
      <div className="px-5 space-y-4">
        {filteredBooks.map((book) => (
          <button
            ...
            className="w-full bg-[#1a2332] rounded-2xl p-4 flex gap-4 items-start hover:bg-[#212d3f] transition-colors"
          >
            ...
            <div className="flex-1 text-left min-w-0">
              <div className="flex items-start justify-between gap-2 mb-1">
                <h3 className="text-white line-clamp-2">{book.title}</h3>
                {book.isFavorite && (
                  <Heart className="w-4 h-4 text-[#f59e0b] fill-[#f59e0b] flex-shrink-0" />
                )}
              </div>
