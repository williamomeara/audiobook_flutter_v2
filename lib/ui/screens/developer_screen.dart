        ...
        children: [
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: (_isBenchmarking || _isAutoTuning) ? null : () => _runParallelAutoTune(settings.selectedVoice),
              icon: _isAutoTuning
                  ? const SizedBox(
                      width: 16,
                      height: 16,
              ...
              label: Text(_isAutoTuning ? 'Auto-Tuning Concurrency...' : 'Auto-tune Parallel Synthesis'),
            ),
          ),
        ],
      ),
...
''';
  ...
  /// Run parallel synthesis benchmark to find optimal concurrency level.
  Future<void> _runParallelAutoTune(String voiceId) async {
    setState(() {
      _isAutoTuning = true;
      _benchmarkResults = 'Starting parallel synthesis auto-tune...\nThis will test concurrency levels 1, 2, and 3.';
      _lastError = null;
    });

    DevLogger.info('[AutoTune] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    DevLogger.info('[AutoTune] STARTING PARALLEL SYNTHESIS AUTO-TUNE');
    DevLogger.info('[AutoTune] Voice: $voiceId');

    try {
      // Get TTS engine and cache
      final routingEngine = await ref.read(ttsRoutingEngineProvider.future);
      final cache = await ref.read(audioCacheProvider.future);
      ...
      // Generate SHORT test text (10 segments for speed)
      const testText = '''
The quick brown fox jumps over the lazy dog. This is a test of parallel synthesis.
...
How razorback-jumping frogs can level six piqued gymnasts! Sixty zippers were quickly picked.
Big July earthquakes confound zany experimental vow. Foxy parsons quiz and cajole the lovably.
...
''';
      ...
      final segments = segmentText(testText);
      DevLogger.info('[AutoTune] Test segments: ${segments.length}');

      final results = <int, _ConcurrencyResult>{};

      // Test each concurrency level
      for (final concurrency in [1, 2, 3]) {
        DevLogger.info('[AutoTune] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        DevLogger.info('[AutoTune] Testing concurrency: $concurrency');

        setState(() {
          _benchmarkResults = '''
🔄 Auto-tuning parallel synthesis...

Testing concurrency level: $concurrency of 3
Segments: ${segments.length}

${results.entries.map((e) => '• Concurrency ${e.key}: ${e.value.totalTimeMs}ms (${e.value.failed} failed)').join('\n')}
''';
        });
        ...
        // Parallel synthesis using a semaphore
        ...
        for (final segment in segments) {
          futures.add(() async {
            ...
            try {
              ...
              failed++;
              DevLogger.error('[AutoTune] Segment failed: $e');
            } finally {
              semaphore.release();
            }
          }());
        ...
        results[concurrency] = _ConcurrencyResult(
        ...
        DevLogger.info('[AutoTune] Concurrency $concurrency: ${totalTime.inMilliseconds}ms, completed: $completed, failed: $failed');
      }

      // Find optimal (fastest with no failures, or least failures)
      final sortedResults = results.values.toList()
        ..sort((a, b) {
          // Prioritize no failures, then fastest time
          if (a.failed == 0 && b.failed > 0) return -1;
          if (a.failed > 0 && b.failed == 0) return 1;
          return a.totalTimeMs.compareTo(b.totalTimeMs);
        });

      final optimal = sortedResults.first;

      DevLogger.info('[AutoTune] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      DevLogger.info('[AutoTune] RESULTS');
      for (final r in results.values) {
        DevLogger.info('[AutoTune] Concurrency ${r.concurrency}: ${r.totalTimeMs}ms (${r.failed} failed)');
      }
      DevLogger.info('[AutoTune] OPTIMAL: Concurrency ${optimal.concurrency}');
      DevLogger.info('[AutoTune] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

      // Calculate speedup
      final sequential = results[1]!;
      final speedup = sequential.totalTimeMs / optimal.totalTimeMs;

      setState(() {
        _benchmarkResults = '''
✓ Parallel Synthesis Auto-Tune Complete!

📊 Results:
${results.values.map((r) => '• Concurrency ${r.concurrency}: ${(r.totalTimeMs / 1000).toStringAsFixed(1)}s ${r.failed > 0 ? '(⚠️ ${r.failed} failed)' : '✓'}').join('\n')}

🏆 OPTIMAL: Concurrency ${optimal.concurrency}
${optimal.concurrency > 1 ? '⚡ Speedup: ${speedup.toStringAsFixed(1)}x vs sequential' : ''}
${optimal.failed > 0 ? '⚠️ ${optimal.failed} failures detected' : ''}

💡 Recommendation:
${optimal.concurrency == 1 
    ? 'Sequential synthesis is best for this device.\nParallel synthesis may cause memory issues or errors.'
    : optimal.concurrency == 2
...
''';
      ...
      DevLogger.error('[AutoTune] ERROR: $e');
      DevLogger.error('[AutoTune] Stack: $st');
      setState(() {
        _lastError = 'Auto-tune failed: $e';
        _benchmarkResults = null;
      });
    } finally {
      setState(() => _isAutoTuning = false);
    }
  }
}

/// Simple semaphore for auto-tuning test.
class _SimpleSemaphore {
  _SimpleSemaphore(this._maxCount);
...
/// Result of a concurrency level test.
class _ConcurrencyResult {
  const _ConcurrencyResult({
